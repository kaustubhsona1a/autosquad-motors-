...
export default function VehicleDetails() {
  ...
  if (!car) {
    ...
    return (
      <div className="min-h-screen bg-transparent flex items-center justify-center p-20 text-zinc-650">
        <Helmet>
          <title>Vehicle Not Found | AutoSquad</title>
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on AutoSquad website. Please share more details.`);
    window.open(`https://wa.me/919769699655?text=${message}`, '_blank');
  };

  const handleWhatsAppPhotos = () => {
    const message = encodeURIComponent(`Hi, I'm interested in the ${car.year} ${car.make} ${car.model} (${car.variant}) listed at ${formatPrice(car.price)} on AutoSquad website. Please share more photos of this vehicle.`);
    window.open(`https://wa.me/919769699655?text=${message}`, '_blank');
  };
  ...
  const renderTechnicalDetails = (isMobile: boolean) => {
    ...
    return (
      <div className="edgy-card-wrapper">
        <div className={`edgy-card bg-[#080808]/90 backdrop-blur-md ${isMobile ? 'p-4 space-y-4' : 'p-10 space-y-8'}`}>
          <div>
            <h2 className={`font-serif font-bold text-[#D4AF37] border-b border-zinc-800 ${isMobile ? 'text-lg mb-4 pb-3' : 'text-2xl mb-8 pb-5'}`}>Technical Details</h2>
            <div className={`grid grid-cols-2 ${isMobile ? 'gap-2' : 'md:grid-cols-4 gap-4'} text-zinc-300`}>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Make</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.make}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Model</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.model}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Year</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.year}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Mileage</p>
                <p className="text-[#FCF6BA] font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.mileage.toLocaleString()} KM</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Fuel Type</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.fuelType}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Transmission</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.transmission}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Ownership</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.ownership}</p>
              </div>
              <div className={`bg-zinc-950/80 border border-zinc-800 edgy-card-sm ${isMobile ? 'p-2.5' : 'p-4'}`}>
                <p className={`${isMobile ? 'text-[9px]' : 'text-[11px]'} tracking-wider uppercase text-zinc-500 mb-1 font-bold font-serif`}>Color</p>
                <p className="text-white font-bold text-xs sm:text-sm md:text-base tracking-wide uppercase font-serif">{car.color}</p>
              </div>
            </div>
          </div>

          {car.description && (
            <div className={`border-t border-zinc-800 ${isMobile ? 'pt-4' : 'pt-8'}`}>
              <h3 className="text-xs sm:text-sm font-bold font-serif text-[#D4AF37] mb-2 sm:mb-4 uppercase tracking-widest">Additional Info</h3>
              <p className={`text-zinc-300 font-light leading-relaxed whitespace-pre-line font-sans ${isMobile ? 'text-xs' : 'text-sm'}`}>{car.description}</p>
            </div>
          )}
        </div>
      </div>
  ...
  const renderPriceBox = (isMobile: boolean) => {
    ...
    return (
      <div className="edgy-card-wrapper">
        <div className={`edgy-card bg-[#080808]/90 backdrop-blur-md relative ${isMobile ? 'p-5' : 'p-10'}`}>
          <div className="absolute top-0 right-0 bg-[#D4AF37] text-black font-serif font-bold tracking-widest uppercase text-[9px] px-3 py-1 edgy-card-sm shadow-md">
            Verified Asset
          </div>
          ...
          <p className={`tracking-widest uppercase text-[#D4AF37] font-serif font-bold ${isMobile ? 'text-[9px] mt-1.5 mb-4' : 'text-[10px] mt-3 mb-8'}`}>{car.variant}</p>
          <div className={`font-bold text-[#FCF6BA] border-b border-zinc-800 font-serif ${isMobile ? 'text-2xl pb-4 mb-4' : 'text-3xl pb-6 mb-4'}`}>{formatPrice(car.price)}</div>

          {car.instagramReel && (
            <div className={`edgy-card-sm bg-gradient-to-r from-[#D4AF37]/10 to-zinc-950/40 border border-[#D4AF37]/30 flex flex-col ${isMobile ? 'mb-4 p-3 gap-2' : 'mb-8 p-4.5 gap-3.5'}`}>
        ...
        <p className={`tracking-wider text-zinc-400 flex items-center font-mono font-semibold ${isMobile ? 'text-[10px] mb-4' : 'text-xs mb-8'}`}>
          <a href="https://maps.app.goo.gl/D6y6jmfmthu22fyY9" target="_blank" rel="noreferrer" className="hover:text-[#D4AF37] transition-colors duration-300 inline-flex items-center">
            <MapPin className="w-4 h-4 mr-2 text-[#D4AF37]" /> Showroom, Agripada
          </a>
        </p>
  ...
  const renderEMICalculator = (isMobile: boolean) => {
    ...
    return (
      <div className="edgy-card-wrapper">
        <div className={`edgy-card bg-[#080808]/90 backdrop-blur-md relative ${isMobile ? 'p-5 space-y-4' : 'p-8 space-y-6'}`}>
          <h2 className={`font-serif font-bold text-[#D4AF37] uppercase tracking-widest border-b border-zinc-800 flex items-center justify-between ${isMobile ? 'pb-2 text-xs' : 'pb-3 text-sm'}`}>
            <span>EMI ESTIMATE CALCULATOR</span>
            <span className="text-[9px] uppercase tracking-widest font-serif text-[#D4AF37]/70">Live</span>
          </h2>
  ...
  const pageTitle = `${car.year} ${car.make} ${car.model} ${car.variant} | AutoSquad`;
  ...
  return (
    <div className="min-h-screen bg-transparent text-zinc-750 py-12 font-sans selection:bg-white selection:text-zinc-950 z-10 relative">
      ...
      <div className="container mx-auto max-w-7xl px-4">
        ...
        <div className="mt-6 md:mt-10 edgy-card-wrapper">
          <div className="edgy-card bg-[#080808]/90 p-5 md:p-10 relative overflow-hidden group backdrop-blur-md">
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity duration-1000">
              <Search className="w-40 h-40 text-[#D4AF37]" />
            </div>
            <h2 className="text-xl md:text-2xl font-serif font-bold text-[#D4AF37] mb-2 sm:mb-3">Certified Pre-Owned Selection</h2>
            <p className="text-zinc-400 mb-2 max-w-xl text-xs md:text-sm font-light leading-relaxed font-sans">
              Every vehicle listed by the AutoSquad platform undertakes a specialized physical inspection covering diagnostic checks, accident/flood history, mileage validation, and comprehensive ownership verification.
            </p>
          </div>
        </div>
          ...
          <div className="bg-zinc-900/90 border border-zinc-800/80 rounded-2xl p-6 sm:p-8 max-w-md w-full relative z-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200 backdrop-blur-xl text-zinc-300">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <button
                  onClick={() => {
                    const msg = `Check out this gorgeous ${car.year} ${car.make} ${car.model} listed at ${formatPrice(car.price)} on AutoSquad!`;
                ...
                <button
                  onClick={() => {
                    ...
                    const body = `Hey, take a look at this exceptional pre-owned ${car.year} ${car.make} ${car.model} listed at ${formatPrice(car.price)} on the AutoSquad website: ${window.location.href}`;
