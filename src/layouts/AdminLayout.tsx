...
export default function AdminLayout() {
  ...
  const { isAdmin, logout, loginAsDealer, user, loading: authLoading } = useAuth();
      ...
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-zinc-300 font-sans relative px-4">
        ...
        <div className="w-full max-w-md bg-zinc-900/60 backdrop-blur-md border border-zinc-900 rounded-3xl p-10 shadow-2xl relative overflow-hidden text-center z-10">
          ...
          <Link to="/" className="inline-flex flex-col items-center mb-8 group">
            {siteConfig.logo ? (
              <img src={siteConfig.logo} alt="AutoSquad" className="h-14 w-auto object-contain mb-3" />
            ) : (
              <div className="w-14 h-14 bg-zinc-900 border border-[#C8A64A]/30 rounded-2xl flex items-center justify-center mb-3">
                <CarFront className="w-6 h-6 text-[#C8A64A]" />
              </div>
            )}
            <h1 className="text-xl font-serif text-white tracking-widest leading-none font-bold uppercase block text-center">
              AUTOSQUAD
            </h1>
            <p className="text-[8px] uppercase tracking-[0.45em] text-[#C8A64A] font-mono mt-1.5 font-semibold">Dealer Portal Lock</p>
          </Link>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <div className="space-y-2 text-left">
              ...
              <div className="relative">
                <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#C8A64A]" />
                <input 
                  id="dealer-email"
                  type="email" 
                  value={email}
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-zinc-950/80 border border-zinc-800 rounded-xl text-left text-sm font-semibold tracking-wider text-white placeholder:text-zinc-700/50 focus:outline-none focus:border-[#C8A64A] transition-all"
                  placeholder="info@autosquad.in"
                  required
                />
              </div>
            </div>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 w-64 bg-zinc-950/95 lg:bg-zinc-950/65 backdrop-blur-md text-white flex-shrink-0 flex flex-col border-r border-white/5 z-50 lg:z-10 transition-transform duration-300 ease-out transform ${
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/5">
          <Link to="/" className="flex items-center space-x-2 group">
            <div className="bg-[#C8A64A] p-1.5 rounded-lg text-zinc-950">
              <CarFront className="w-5 h-5" />
            </div>
            <div className="flex flex-col items-start leading-none">
              <span className="font-serif text-sm tracking-widest text-white font-bold uppercase leading-none">AUTOSQUAD</span>
              <span className="font-mono text-[8px] tracking-[0.3em] font-bold text-[#C8A64A] mt-0.5">SHOWROOM</span>
            </div>
          </Link>
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
          <div className="flex items-center p-4 pb-2">
            ...
            <div className="leading-tight overflow-hidden">
              <p className="text-xs font-semibold truncate text-white" title={user?.user_metadata?.full_name || user?.displayName || user?.email?.split('@')[0] || "AutoSquad Partner"}>
                {user?.user_metadata?.full_name || user?.displayName || user?.email?.split('@')[0] || "AutoSquad Partner"}
              </p>
              <p className="text-[10px] text-[#C8A64A] font-mono uppercase tracking-wider">
                ● SUPABASE READY
              </p>
            </div>
          </div>
        ...
        <header className="h-16 bg-zinc-950/40 backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 md:px-8 flex-shrink-0 gap-4">
          <div className="flex items-center space-x-3 flex-1 min-w-0">
            ...
            <div className="sm:hidden flex flex-col items-start leading-none pointer-events-none">
              <span className="font-serif text-[11px] tracking-widest text-white font-bold uppercase">AUTOSQUAD</span>
              <span className="font-mono text-[7px] tracking-[0.2em] font-bold text-[#C8A64A] mt-0.5">DEALER</span>
            </div>
          </div>
